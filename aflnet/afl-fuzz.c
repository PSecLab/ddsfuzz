...
int send_over_network_fastdds()
{
  ...
  serv_addr.sin_addr.s_addr = inet_addr("239.255.0.1");
...
int send_over_network_cyclonedds()
{
  ...
  serv_addr.sin_family = AF_INET;
  serv_addr.sin_port = htons(12400);
  serv_addr.sin_addr.s_addr = inet_addr("239.255.0.1");
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if(dds_mode){
    if (dumb_mode == 1 || no_forkserver) {
      if (use_net) {
        if(strcmp(argv[0], "./FastddsFuzz")==0){   send_over_network_fastdds();}
        else if(strcmp(argv[0], "./CycloneddsFuzz")==0){send_over_network_cyclonedds();}
        else if(strcmp(argv[0], "./OpenddsFuzz")==0){send_over_network_opendds();}
...
void dds_usage() {
  ...
  fastdds_argv[0] = malloc(strlen("./FastddsFuzz")+1);
  cyclonedds_argv[0] = malloc(strlen("./CycloneddsFuzz")+1);
  opendds_argv[0] = malloc(strlen("./OpenddsFuzz")+1);
  strcpy(fastdds_argv[0], "./FastddsFuzz");
  strcpy(cyclonedds_argv[0], "./CycloneddsFuzz");
  strcpy(opendds_argv[0], "./OpenddsFuzz");
